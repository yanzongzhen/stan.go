...
import (
	...
	"errors"
	"fmt"
	"github.com/yanzongzhen/stan.go/utils"
	"github.com/yanzongzhen/stan.go/variable"
	"sync"
	"time"
...
func (sc *conn) GetSession() *variable.Subscribe {
	return sc.Session
}

func (sc *conn) GetServer() *variable.WatchServer {
	sc.RLock()
	nc := sc.server
	if sc.server.Running {
		nc = nil
	}
	sc.RUnlock()
	return nc
...
// Optimized and upgraded version
func (sc *conn) QueueSubscribePromote(subject string, cb MsgHandler, options ...SubscriptionOption) (Subscription, error) {
	group, err := sc.PreHandler(subject, WrapperSync(cb, subject))
	if err != nil {
		return nil, err
	}
	return sc.subscribe(subject, group, WrapperCb(sc, cb, subject), options...)
}

func WrapperCb(sc *conn, cb MsgHandler, t string) MsgHandler {
	fd := func(msg *Msg) {
		cb(msg)
		go func() {
			sc.GetSession().SendMsg(t, msg.Data)
		}()
	}
	return fd
}

func WrapperSync(cb MsgHandler, t string) variable.MessageHandler {
	fd := func(msg []byte) {
		var s map[string]string
		_ = json.Unmarshal(msg, &s)
		msg1 := &Msg{}
		msg1.Data = []byte(s["content"])
		msg1.Subject = t
...
func (sc *conn) PreHandler(subject string, syncBack variable.MessageHandler) (string, error) {
	// 启动同步服务
	ip, _ := utils.ExternalIP()
	port, _ := utils.GetFreePort()
	// 标示本地链接
	sc.Session.LocalIP = ip.String()
	sc.Session.LocalPort = port
	sc.server = &variable.WatchServer{Host: ip.String(), Port: port, MsgChan: make(chan []byte)}
	sc.server.Run(syncBack)
	var group string
	go func() {
		sc.GRW.Add(1)
		sc.Session.HealthCheck(sc.GRW)
	}()
	// 设备信息 获取 分组
	msg := make(map[string]interface{})
	msg["host"] = ip.String()
	msg["port"] = port
	msg["client_id"] = sc.clientID
	msg["topic"] = subject
	d, _ := json.Marshal(msg)
	message, err := sc.nc.Request(DefaultRequestTopic, d, time.Second*1)
	if err != nil {
		return "", err
	}
	if message != nil {
		...
		group = m[0]["group"].(string)
		sc.Session.UpdateClient(subject, sc.clientID, m)
		key := fmt.Sprintf("T:%vG:%v", subject, group)
		sub, _ := sc.nc.Subscribe(key, func(msg *nats.Msg) {
			mm := make([]map[string]interface{}, 0)
			_ = json.Unmarshal(msg.Data, &mm)
			sc.Session.UpdateClient(subject, sc.clientID, mm)
		})
		sc.syncSubMap[subject] = sub
	}
	return group, nil
}
