...
type Conn interface {
	...
	// boom upgrade version
	QueueSubscribePromote(subject string, cb MsgHandler, opts ...SubscriptionOption) (Subscription, error)

	// get Session
	GetSession() *variable.Subscribe

	// get server
	GetServer() *variable.WatchServer
...
type conn struct {
	...
	closed           bool
	ping             pingInfo
	Session          *variable.Subscribe
	GRW              *sync.WaitGroup
	server           *variable.WatchServer
	syncSubMap       map[string]*nats.Subscription
}
...
func Connect(stanClusterID, clientID string, options ...Option) (Conn, error) {
	...
	c := conn{
		...
		syncSubMap:      make(map[string]*nats.Subscription),
		GRW:             &sync.WaitGroup{},
	}
	...
	c.Session = &variable.Subscribe{Clients: make(map[string][]*variable.InnerClient), PingInterval: c.opts.PingInterval}
...
func (sc *conn) Close() error {
	...
	// close Connect in client
	if sc.Session != nil {
		sc.Session.Close()
	}
	if sc.server != nil {
		sc.server.Serve.Stop()
	}
	if sc.syncSubMap != nil {
		for _, v := range sc.syncSubMap {
			_ = v.Unsubscribe()
		}
		sc.syncSubMap = make(map[string]*nats.Subscription)
	}
