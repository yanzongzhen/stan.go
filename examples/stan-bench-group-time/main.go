...
func runPublisher(startwg, donewg *sync.WaitGroup, url string, opts []nats.Option, clusterID string, numMsgs, msgSize int, sync bool, pubID string, maxPubAcksInflight int) {
	...
	if !sync {
		...
		for i := 0; i < numMsgs; i++ {
			m := fmt.Sprintf("%s$%v", msg, time.Now().UnixNano())
			_, err := snc.PublishAsync(subj, []byte(m), acb)
			if err != nil {
				log.Fatal(err)
			}
		}
...
func runSubscriber(startwg, donewg *sync.WaitGroup, url string, opts []nats.Option, clusterID, subID, queue string, numMsgs, msgSize int, ignoreOld bool) {
	...
	isQueue := queue != ""
	received := 0
	timeDiff := make([]int64, 0)

	mcb := func(msg *stan.Msg) {
		getMsg := string(msg.Data)
		m := strings.Split(getMsg, "$")
		t, _ := strconv.ParseInt(m[len(m)-1], 10, 64)
		now := time.Now().UnixNano()
		diff := now - t
		timeDiff = append(timeDiff, diff)
		received++
		if received == 1 {
			ch <- time.Now()
		}
	...
	if ignoreOld {
		sub, err = snc.QueueSubscribePromote(subj, mcb)
	} else {
		sub, err = snc.QueueSubscribePromote(subj, mcb, stan.StartWithLastReceived())
	}
	if err != nil {
	...
	newArray := make([]int, 0)
	for _, val := range timeDiff {
		a := int(val) / 1e6
		newArray = append(newArray, a)
	}
	max, min, avg := calculate(newArray)
	log.Printf("Max %vms | Min %vms | AVG %vms", max, min, avg)
...
func calculate(a []int) (int, int, float32) {
	max := a[0]
	min := a[0]
	sum := 0
	for i := 0; i < len(a); i++ {
